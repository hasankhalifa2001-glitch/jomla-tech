...
const newProductRowSchema = z
  .object({
    ...
    // [FIX — reverted] conversionFactor is a plain positive decimal here,
    // NOT locked to 1 at the schema level. This array holds BOTH
    // "brand-new product" rows (whose unit must be the base unit, factor
    // 1) AND "additional packaging unit on an existing product" rows
    // (whose unit must NOT be factor 1) — csv-parser.ts's own preview/
    // commit logic documents this explicitly (see its "known, documented
    // limitation" note on newProductsCount). Which case a given row is
    // can only be determined at commit time, against the product's real
    // (name, category) match in the live DB — never knowable here at the
    // route/schema layer in isolation. The actual base-unit-vs-1 /
    // duplicate-factor rule is enforced correctly and authoritatively by
    // validatePackagingUnits() inside commitCsvImport(), against the
    // product's REAL current units — that is the single source of truth
    // for this rule, not a blanket schema-level check here, which would
    // (and previously did, in an earlier revision of this file) reject
    // every legitimate additional-unit CSV row outright.
